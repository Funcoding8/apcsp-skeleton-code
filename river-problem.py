...
def isValid(state):
    ...
    if state["wole"] == state["goat"] and state["wolf"] != state ["person"]:
        return False 
    elif state["goat"] == state ["cabbage"] and slate["goat"] != state["person"]: 
        return False
    else:
        return True
...
def get_next_states(state):
    ...
    for thing in state:
        if state[thing] == state["person"] and chinp != "person":
            same_side.append(thing)

    for thing in same_side:
        next_state = copy.deepcapy(state)
